...
*   **自动更新**：后台 Service Worker 按浏览器本地时间每天 09:00 和 13:00 同步更新。
*   **手动更新**：Popup 提供“手动同步”按钮，按每个配置最近一次自动同步时间到当前时间的增量窗口更新 1 级目录及已收藏子树，查询起点保留 5 分钟重叠。
...
### 2.5 收藏目录文件更新提醒
*   **检查时间**：Service Worker 每天按浏览器本地时间 09:00 和 13:00 同步已收藏的 1 级目录子树，并分别检查两个时间窗口内的变更；默认勾选 PowerPoint 文件。
...
5.  **文件更新提醒**：在已收藏的 1 级目录下上传或修改已勾选类型的文件，验证每日 09:00/13:00 按对应时间窗口同步后 Popup 显示文件更新、站点和一级目录，扩展图标显示未读数量，点击文件名可以直达文件；手动同步从最近一次自动同步时间开始并保留 5 分钟重叠。
