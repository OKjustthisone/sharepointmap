...
*   **自动更新**：后台 Service Worker 按浏览器本地时间每天 09:00 和 13:00 同步更新；如果错过了一个或多个自动窗口，则从该配置最近一次成功的自动同步时间追溯补偿。
*   **手动更新**：Popup 提供“手动同步”按钮，按每个配置最近一次自动同步时间到当前时间的增量窗口更新 1 级目录及已收藏子树，查询起点保留 5 分钟重叠，并生成文件更新提醒。
...
*   **更新机制**：每天 09:00 检查前一天 13:00 至当天 09:00 的变更，每天 13:00 检查当天 09:00 至 13:00 的变更，并更新已收藏的 1 级目录及其整棵子树。
...
graph TD
    ...
    B --> C[(本地缓存: 1级路径)]
    C --> D{每日 09:00/13:00 或手动同步?}
    D -- 是 --> B
    
    C --> E[用户收藏某个 1 级文件夹]
    E --> F[后台启动递归任务: 获取该文件夹下所有层级的子目录与文件]
    F --> G[(本地缓存: 完整子树数据)]
    G --> H{每日 09:00/13:00 或手动同步?}
    H -- 是 --> F
...
### 2.5 收藏目录文件更新提醒
*   **检查时间**：Service Worker 每天按浏览器本地时间 09:00 和 13:00 同步已收藏的 1 级目录子树，并分别检查两个时间窗口内的变更；错过窗口时自动追溯到最近一次成功的自动同步时间；默认勾选 PowerPoint 文件。
*   **通知类型设置**：在设置页可勾选 PowerPoint、Word、Excel、PDF；仅勾选的文件类型会生成更新提醒，全部取消则关闭文件更新提醒。
*   **提醒内容**：发现新上传或 `Modified` 时间发生变化的文件后，在 Popup 顶部以“文件更新”卡片展示文件名、更新类型、时间、所在站点和一级目录，标题不显示额外图标。
*   **直达文件**：通知中的文件名和“打开文件”按钮可直接打开 SharePoint 文件链接。
*   **未读徽标**：Chrome 扩展图标显示未读提醒数量；用户可逐条或一键标记已读，提醒按文件 ID 和更新时间去重并保留最近 100 条。
...
5.  **文件更新提醒**：在已收藏的 1 级目录下上传或修改已勾选类型的文件，验证每日 09:00/13:00 按对应时间窗口同步后 Popup 显示文件更新、站点和一级目录，扩展图标显示未读数量，点击文件名可以直达文件；错过自动窗口时应补偿通知，手动同步从最近一次自动同步时间开始并保留 5 分钟重叠且生成通知。
6.  **通知类型设置**：分别勾选和取消 PowerPoint、Word、Excel、PDF，验证配置保存、编辑回填和同步通知过滤均符合选择结果。
