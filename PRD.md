...
### 2.5 收藏目录 PPT 更新提醒
*   **检查时间**：Service Worker 每天按浏览器本地时间 10:00 同步已收藏的 1 级目录子树，并检查其中的 PPT、PPTX、PPTM、PPS、PPSX、PPSM、POT、POTX、POTM 文件。
*   **提醒内容**：发现新上传或 `Modified` 时间发生变化的演示文稿后，在 Popup 顶部以“📅 会议通知”卡片展示文件名、更新类型、时间和所在目录。
...
5.  **PPT 更新提醒**：在已收藏的 1 级目录下上传或修改 PPT 文件，验证每日 10:00 同步后 Popup 显示会议通知、扩展图标显示未读数量，点击文件名可以直达文件。
